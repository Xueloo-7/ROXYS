    ...
    <table style="margin-top: 100px;" border="1" cellspacing="0" cellpadding="8">
        ...
        <br>
            ...
            <tr data-id="<?= $product['id'] ?>">
                ...
                <td>
                    <img src="<?= BASE_URL.$product['image_url'] ?>" alt="product image" style="width: 100px; height: 100px; object-fit: cover;">
                    <input type="file" name="image" accept="image/*" style="display: none;">
                    <button type="button" class="upload-image-btn">Upload Image</button>
                </td>
...
$('.save-btn').click(function(e) {
    ...
    formData.append('price', row.find('[name="price"]').val());
    formData.append('discount', row.find('[name="discount"]').val());
    formData.append('stock', row.find('[name="stock"]').val());
    formData.append('description', row.find('[name="description"]').val());
    formData.append('details', row.find('[name="details"]').val());
    formData.append('image_url', row.find('[name="image_url"]').val());
    formData.append('category', row.find('[name="category"]').val());
    formData.append('sizes', row.find('[name="sizes"]').val());
    formData.append('colors', row.find('[name="colors"]').val());

    // Add image file if exists
    const imageInput = row.find('[name="image"]')[0];
...
$('input[type="file"]').change(function() {
    ...
    $(this).siblings('.upload-image-btn').text(fileName || 'Upload Image');
});
</script>